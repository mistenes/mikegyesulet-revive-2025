...
export default function AdminResetPassword() {
  ...
  useEffect(() => {
    if (!token) {
      toast.error('Hiányzó jelszó-visszaállító token');
      navigate('/auth');
    }
  }, [token, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Új jelszó beállítva. Jelentkezz be az új adatokkal.');
      navigate('/auth');
    } catch (error) {
      const message = (error as Error)?.message || 'Nem sikerült visszaállítani a jelszót';
      toast.error(message);
    } finally {
    ...
    <div className="min-h-screen bg-gradient-subtle flex items-center justify-center px-4">
      <Card className="w-full max-w-lg p-8 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">Állíts be új jelszót a belépéshez.</p>
        </div>
        ...
        <div className="bg-muted p-4 rounded-lg text-sm text-muted-foreground space-y-2">
          <div className="flex items-center gap-2">
            ...
            <p>Ne oszd meg ezt a linket senkivel.</p>
          </div>
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 text-primary" />
